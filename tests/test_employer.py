...
def test_employer_my_jobs_requires_login(employer_client):
    response = employer_client.get('/employer/my_jobs', follow_redirects=True)
    assert b'Login' in response.data or response.status_code == 403

def test_post_job(employer_client):
    resp = employer_client.post('/employer/post_job', data={
        'title': 'Employer Job',
...
def test_edit_job(employer_client):
    ...
    db.session.add(job)
    db.session.commit()
    resp = employer_client.post(f'/employer/jobs/{job.id}/edit', data={
        'title': 'EditedJob',
        'description': 'desc',
        'location': 'Remote',
        'company': 'EmpCo',
...
def test_delete_job(employer_client):
    ...
    db.session.add(job)
    db.session.commit()
    resp = employer_client.post(f'/employer/jobs/{job.id}/delete', follow_redirects=True)
    assert b'deleted' in resp.data or resp.status_code == 200

def test_post_job_redirect_employer(employer_client):
    resp = employer_client.get('/employer/post-job-redirect', follow_redirects=True)
...
def test_new_job_get(employer_client):
    resp = employer_client.get('/employer/jobs/new')
...
def test_new_job_post_invalid(employer_client):
    ...
    resp = employer_client.post('/employer/jobs/new', data={}, follow_redirects=True)
    assert b'Job' in resp.data or resp.status_code == 200

def test_my_jobs(employer_client):
    resp = employer_client.get('/employer/my_jobs')
    assert resp.status_code == 200
...
def test_job_applications_permission(employer_client):
    ...
    db.session.commit()
    resp = employer_client.get(f'/employer/jobs/{job.id}/applications')
    assert resp.status_code == 200
...
def test_job_applications_unauthorized(employer_client):
    ...
    db.session.add(job)
    db.session.commit()
    resp = employer_client.get(f'/employer/jobs/{job.id}/applications', follow_redirects=True)
...
def test_update_application_permission(employer_client):
    ...
    db.session.add(app_obj)
    db.session.commit()
    resp = employer_client.post(f'/employer/applications/{app_obj.id}/update', data={'status': 'reviewed'}, follow_redirects=True)
...
def test_update_application_invalid_status(employer_client):
    ...
    db.session.add(app_obj)
    db.session.commit()
    resp = employer_client.post(f'/employer/applications/{app_obj.id}/update', data={'status': 'invalid'}, follow_redirects=True)
